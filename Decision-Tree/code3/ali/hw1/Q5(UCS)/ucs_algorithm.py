  ...
  def search(self):
    ...
    while True:
      ...
      if selected_node == self.target:
        """ Put your code here!!!"""
        """ Uncomment the lines below and fill in the blanks by using the implemented functions"""
      ...
      if len(new_nodes) > 0:
        for node in new_nodes :
          heuristic_value = self.calculate_distance(selected_node , node)
          if node not in self.opened and node not in self.closed :
            node.heuristic_value = heuristic_value
            self.insert_to_list("open" , node )
          elif node in self.opened and node.parent != selected_node :
            old_node = self.get_old_node(node.value)
            if old_node.heuristic_value > heuristic_value :
              node.heuristic_value = heuristic_value
               
        """ For every new node, first calculate its heuristic value
         If it was not in closed or opened nodes lists, expand it
         If it was already opened and its parent was not the selected node, use get_old_node function and
         in the end compare heuristic values and open the node if it had the right conditions"""
      """ Put your code here!"""
